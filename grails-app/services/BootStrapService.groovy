...
class BootStrapService {
	...
	private sql(){
	 ...
	 def versionMap(appVersion){
		...
		return [majorVersion : _l(appVersionArray[0]),
		midVersion : _l(appVersionArray[1]),
		minorVersion : _l(appVersionArray[2])]
	 }

	def execDdl(sql){dataSource.connection.prepareStatement(sql).executeUpdate();log.info("[$sql] OK ")} 	 
	def updateData(sql){def rowsUpdated = dataSource.connection.prepareStatement(sql).executeUpdate();log.info("[$sql] : $rowsUpdated rows updated.")}
