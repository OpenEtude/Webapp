...
<html>
    <head>
        ...
        <title>Rapprochement du compte ${compteBancaire}</title>
</head>
    <body>
        ...
        <div class="nav">
            ...
            <span class="menuButton"><g:link controller="ecriture" action="affect" id="${compteBancaire.id}"><span class="database_edit ico"></span>Affectation au compte ${compteBancaire}</g:link></span>
        </div>
		<div class="searchbox">
<g:form border="0" url='[controller: "ecriture", action: "validate"]'   id="ecritureSearch" name="ecritureSearch"  accept-charset="UTF-8"  method="get" style="display:inline;">
			<etude:keyword name="q" accesskey="s" id="keybox" value="${params.q}" hint="${'Montant, Piece ou Commentaire'.encodeAsJavaScript()}"/>
</g:form>
		</div>
        <div class="body">
			<h1>Rapprochement du compte ${compteBancaire}</h1>
            <g:if test="${ecritureDossierMap.keySet().empty}">
				<i>Toutes les &eacute;critures ont &eacute;t&eacute; rapproch&eacute;es</i>
            </g:if>
            <g:else>
				<i>${count} &eacute;critures non rapproch&eacute;es : </i>
			...
			<div class="buttons">
				...
				<g:actionSubmit class="validate" onclick="return confirm('Etes-vous sur de vouloir valider?');" action="submitToValidate" value="Valider la s&eacute;lection" /></span>
				</div>
			</div>
            ...
            <div class="list">
                <table>
                    <thead>
                        <tr>
                   	        <g:sortableColumn action="validate" id="${compteBancaire.id}" property="moyenPaiement" title="M.P" params="${sortParams}"/>

							<g:sortableColumn action="validate" id="${compteBancaire.id}" property="typeEcriture" title="Libell&eacute;" params="${sortParams}"/>
		
							<g:sortableColumn action="validate" id="${compteBancaire.id}" property="montant" title="D&eacute;bit" params="${sortParams}"/>
						
							<g:sortableColumn action="validate" id="${compteBancaire.id}" property="montant" title="Cr&eacute;dit" params="${sortParams}"/>
						
                   	        <g:sortableColumn action="validate" id="${compteBancaire.id}" property="dateValeur" title="Date D&eacute;p&ocirc;t" params="${sortParams}"/>
                        
                   	        <g:sortableColumn action="validate" id="${compteBancaire.id}" property="dateMouvement" title="Date Encaissement" params="${sortParams}"/>
                        
                   	        <g:sortableColumn action="validate" id="${compteBancaire.id}" property="pieceComptable" title="Pi&egrave;ce Comptable" params="${sortParams}"/>
