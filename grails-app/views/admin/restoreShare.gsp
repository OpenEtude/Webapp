...
<html>
    <head>
        ...
        <title>Restitution de sauvegarde de fichier(<etude:month num="${month}"/> ${year})</title>
    </head>
    <body>
        <div class="nav">
            ...
            <span class="menuButton"><mymodal:createLink sclass="database_delete" controller="sauvegarde" action="critereDeleteAll" title="Suppression de details d&apos;sauvegarde pour lib&eacute;rer de l&apos;espace" linkname="Nettoyer l&apos;historique" width="600"/></span>
        </div>
        <div class="body">
            <h1>Historique de la sauvegarde du partage de fichiers(<etude:month num="${month}"/> ${year})</h1>
			...
			<g:form accept-charset="UTF-8" controller="admin" method="post" >
				...
				<div>
                    ...
                    <g:each in="${entry.value}" status="i" var="sauvegarde">
                        <li class="${(i % 2) == 0 ? 'odd' : 'even'} clickable" onclick="$('radio${j}x${i}').checked=true;" style="padding-left:7em;">
                            <span><g:formatDate date="${sauvegarde.date}" format="HH:mm"/> <i>${sauvegarde.filename}</i> <span class="tip"><g:formatNumber number="${sauvegarde.size / (1024*1024)}" format="###,##0.00 Mo" /></span></span> <g:link class="popupaction num boldlink" controller='admin' action='download' title="T&eacute;l&eacute;charger ce fichier" params="[filetype:'filebackup',filename:sauvegarde.filename]"><span class="database_save ico"></span>T&eacute;l&eacute;charger</g:link>
                        </li>
                    </g:each>
                    </g:each>
                    </ul>
			...
			<div class="myWidget" style="min-height:600px;">
				<h2>Sauvegardes de fichier</h2>
            <span class="tip">Aucune sauvegarde de fichier n'a &eacute;t&eacute; trouv&eacute;e pour <etude:month num="${month}"/> ${year}</span>
			</div>
            </g:else>
			</td>
			</tr>
			...
			<g:else>
			<div class="dialog">
            <span class="tip">Aucune sauvegarde de fichier n'a &eacute;t&eacute; trouv&eacute;e</span>
            </g:else>
			</div>
        </div>
    </body>
