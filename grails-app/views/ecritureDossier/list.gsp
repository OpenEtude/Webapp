...
<html>
    <head>
		...
		<g:else>
		<title>Liste des Ecritures de Dossier</title>
		</g:else>
    </head>
    <body>
        <div class="nav">
...
<g:form border="0" url='[controller: "ecriture", action: "simpleSearch"]'   id="ecritureSearch" name="ecritureSearch"  accept-charset="UTF-8"  method="get" style="display:inline;">
        ...
        <div class="body">
            ...
            <g:else>
            <h1>Liste des Ecritures de Dossier</h1>
            </g:else>
            ...
            <div class="list">
                <table>
					...
					<% i++ %>
						<etude:tr ctrl="ecritureDossier" id="${ecritureDossier.id}" class="${(ecritureDossier.typeEcriture?.categorieEcriture?.id==1?'frais': ecritureDossier.typeEcriture?.categorieEcriture?.id==2?'prix':'')+((i % 2) == 0 ? 'odd' : 'even')}">
