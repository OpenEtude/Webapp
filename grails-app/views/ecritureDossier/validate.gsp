...
<html>
    <head>
        ...
        <title>Validation des &eacute;critures en cours</title>
    </head>
    <body>
		...
		<div class="searchbox">
<g:form border="0" url='[controller: "ecritureDossier", action: "validate"]'   id="ecritureSearch" name="ecritureSearch"  accept-charset="UTF-8"  method="get" style="display:inline;">
			<etude:keyword name="q" accesskey="s" id="keybox" value="${params.q}" hint="${'Montant, Piece ou Commentaire'.encodeAsJavaScript()}"/>
</g:form>
		</div>
        <div class="body">
			<h1>Validation des &eacute;critures en cours</h1>
            <g:if test="${ecritureDossierMap.keySet().empty}">
				<i>Toutes les &eacute;critures ont &eacute;t&eacute; valid&eacute;es</i>
            </g:if>
            <g:else>
				<i>${count} &eacute;critures &agrave; valider : </i>
			<g:form accept-charset="UTF-8" controller="ecritureDossier" method="post" >
			<input type="hidden" name="encoursSize" value="${encoursSize}" />
			<div class="buttons">
            ...
            <div class="list">
                <table>
                    ...
                    <g:each in="${ecritureDossierMap}" var="entry">
                     <tr class="tip">
                        <td colspan="8"><h2><etude:linkDossier dossier="${entry.key}"/></h2></td>
                     </tr>
