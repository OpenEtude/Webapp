...
<html>
    <head>
        ...
        <title>Saisie Ecriture de Dossier</title>         
    </head>
    <body>
        ...
        <div class="body">
            <h1>Nouvelle Ecriture <g:if test="${params.type}">(${params.type})</g:if> de dossier</h1>
                ...
                <div class="dialog">
                    <table>
                        <tbody>
                            ...
                            <tr class='prop'><td valign='top' class='name'><label for='dossier'>Dossier:</label></td><td valign='top' class='value ${hasErrors(bean:ecritureDossier,field:'dossier','errors')}'>
							<g:if test="${ecritureDossier?.dossier?.id}">
								${ecritureDossier?.dossier?.encodeAsHTML()}
								<input type="hidden" name="dossier.id" value="${params.get('dossier.id')}"/>
								<input type="hidden" name="type" value="${params.get('type')}"/>
							</g:if>
							<g:else>
							<g:select optionKey="id" from="${lovDossier}" name='dossier.id' value="${ecritureDossier?.dossier?.id}"  noSelection="['null':'']"></g:select>
							</g:else>
                            ...
                            <tr class='prop'><td valign='top' class='name'><label for='typeEcriture'>Mode de paiement:</label></td><td valign='top' class='value ${hasErrors(bean:ecritureDossier,field:'moyenPaiement','errors')}'><g:select optionKey="id" from="${MoyenPaiement.list()}" name='moyenPaiement.id' value="${ecritureDossier?.moyenPaiement?.id}" ></g:select></td></tr>
                            ...
                            <tr class='prop'><td valign='top' class='name'><label for='acte'>Acte:</label></td><td valign='top' class='value ${hasErrors(bean:ecritureDossier,field:'acte','errors')}'>
							<g:if test="${ecritureDossier?.dossier?.id}">
							<g:select optionKey="id" from="${Acte.findAllWhere(dossier:ecritureDossier.dossier)}" name='acte.id' value="${ecritureDossier?.acte?.id}" noSelection="['null':'']"></g:select>
							</g:if>
							<g:else>
							<g:select optionKey="id" from="${lovActe}" name='acte.id' value="${ecritureDossier?.acte?.id}" noSelection="['null':'']"></g:select>
							</g:else>
							</td></tr>
							<g:if test="${categ==1l}">
