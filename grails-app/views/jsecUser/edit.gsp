...
<html>
    ...
    <body>
        ...
        <div class="body">
            ...
            <g:form accept-charset="UTF-8"   controller="jsecUser" method="post" >
                <input type="hidden" name="id" value="${jsecUser?.id}" />
                <div class="dialog">
                    <table>
                        <tbody>
				            <tr class='prop'><td valign='top' class='name'><label for='username'>Nom de l'utilisateur:</label></td><td valign='top' class='value ${hasErrors(bean:jsecUser,field:'username','errors')}'><input type="text" id='username' name='username' value="${jsecUser?.username?.encodeAsHTML()}"/><input type="hidden" id='passwordHash' name='passwordHash' value="${jsecUser?.passwordHash?.encodeAsHTML()}"/>
	                    ...
	                    <g:each in="${allRoles}" var="role" status="i">
	                        <tr>
	                            ...
	                            <td valign="top">
									...
									<g:checkBox name="check${i}" value="${role.id in roles}"/>
